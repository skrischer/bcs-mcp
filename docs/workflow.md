...
- GitHub repo: `skrischer/bcs-mcp`
- Base / integration branch: `main`
- GitHub Project board: `https://github.com/users/skrischer/projects/12`
  (number `12`) — mandatory; the loops' queue and status display. Status values:
  `Todo`, `In Progress`, `Done` — a status display, **not** a claim or lock (see
  Orchestration).
- Board field-ID recipe (so the loops set status without re-discovering it):
  ProjectV2 Status field `PVTSSF_lAHOBLauTs4BeNDSzhYpA9g`; option IDs — Todo
  `f75ad846`, In Progress `47fc9ee4`, Done `98236657`. Re-derive with
  `gh project field-list 12 --owner skrischer --format json --jq
...
- All implementation and docs work happens in a worktree — never in the main
  ...
  fast-forward pulls.
- Path convention: `../bcs-mcp-worktrees/<branch-with-slashes-as-dashes>`.
...
- Bootstrap: `pnpm install && cp ../bcs-mcp/.env .` (install deps from the
  lockfile, copy the secrets file from the main checkout into the worktree)
- Verify: `pnpm verify` (`tsc --noEmit && vitest run`) — measured duration: `~4s`
- Test: `pnpm test` (`vitest run`)
- Build: `pnpm build` (`tsup`)

Verify is the per-iteration gate: run it after every change set and fix until
green. Run Build additionally before opening an app-affecting PR. Test is
included in Verify; there is no separate machine check gap.
